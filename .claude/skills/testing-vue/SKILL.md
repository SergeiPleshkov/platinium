...
deliberate violations to disk, lints them, and asserts each is rejected. It **also** asserts that
...
- **`unstubGlobals` clears `beforeAll` stubs.** Global stubs go in `beforeEach` or they survive
  exactly one test.
- **`ResizeObserver` must be stubbed as a class**, not an arrow function, because PrimeVue calls it
