...
access — its deliverable is `plans/code-review.md`.
...
- A feature importing another feature's internals (anything other than its `index.ts`) —
  blocking.
...
- Business logic in a `.vue` `<script setup>` past ~120 lines — flag with the line count.
...
### 8. Tests
- Non-trivial logic in the diff — a composable with branches, a store action, a utility, a
  schema rule — with no accompanying test. Blocking. Pure presentational markup is exempt.
