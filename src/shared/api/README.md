...
That single choke point is what makes authentication, cancellation, timeouts and error
normalisation properties of the *layer* rather than things each call site has to remember.
...
`error.response?.status`, never tell an `AxiosError` from a thrown string, and never render
`[object Object]`.
...
`if (error.isValidation)`, and the day the backend starts returning 400 for validation the
answer changes in one place.

**Messages are written for users, not for logs.** The server's own message wins when it sent
one — it knows which event blocked the delete. Otherwise a status-keyed table supplies the
copy, and it never leaks a status code or a stack trace into the UI.
...
**401 is handled once.** A response interceptor ends the session centrally, with `/auth/login`
...
becomes `kind: 'aborted'` and is dropped rather than surfaced, which is what makes debounced
...
`status[]=draft&status[]=paused`; the handlers — and every conventional REST backend — expect
the repeated form. `buildQueryString` also drops empty values, so a cleared filter leaves no
trace in the URL, and `serialiseListQuery` writes the reserved keys last so a filter named
`page` cannot hijack pagination.
...
Each feature implements it and widens it by intersection for what is genuinely its own:
...
**It is an interface, not a factory, and that was measured.** Written out across three
entities, a `createResource()` factory came to 62 lines against 25 for direct calls — 2.5× the
...
Ids are `encodeURIComponent`-wrapped, `withSignal` keeps `exactOptionalPropertyTypes` happy,
and nothing here needs a `try`/`catch` — the store handles `ApiError`, and there is only one
kind.

## How stores consume it

Two rules, applied consistently across all three entity stores:

- **`fetchList` never throws.** A failed list is a state the page renders — error panel, retry
  button — not an exception every caller must remember to catch. Aborts are swallowed.
- **Mutations always rethrow**, through `asApiError(caught, fallback)`. The caller is a form
  that must decide whether to close, and a 422 carries field errors only it can place.

## Swapping the transport

The layer earned its keep once already: the first implementation was a hand-rolled `fetch`
wrapper. Replacing it with axios changed **one file** — `http.ts` — and every test in this
layer passed unmodified. That is the evidence the boundary is drawn in the right place.

## Tests

[`http.spec.ts`](http.spec.ts) covers it against MSW rather than a stub: header injection, the
401 hook and its login exemption, array and empty-value serialisation, 204 handling, blob
responses, timeout and network normalisation, abort classification, and 422 field-error
extraction. [`tests/mock-api/`](../../../tests/mock-api/) then tests the backend's contract
separately, without this client in the way.
